...
local Texte1 = "Add TRACK, Option[ALT] lets you delete Tracks."
local Texte2 = "ADD Spacer, Option[ALT] lets you delete Spacer after selected Tracks"
...
-- CONFIGURATION
------------------------------------------------------------
local conf = { number_x = 52, palette_y = 1, spacing = 4}
...
--------------------------------------------------------------------------------------------------------
----------------------    Partie a coller avant le Loop() pour vignette      --------------------------
--------------------------------------------------------------------------------------------------------
-- 🔹 URL cible
local lien = "https://github.com/Geeksound/Reaper_Scripts-Mariow"
...
do
  ...
  if file then
    file:close()
    image = reaper.ImGui_CreateImage(image_path)
  else
    image = nil -- image non trouvée, pas de crash
  end
end

-- 🔸 Fonction : ouvrir le lien
function ouvrir_lien()
  if reaper.CF_ShellExecute then
    reaper.CF_ShellExecute(lien)
  else
...
reaper.ShowMessageBox(
  ...
  ----------- End Refresh&Prefix --------------
    ...
    reaper.ImGui_SameLine(ctx)
    if reaper.ImGui_Button(ctx, 'search Tracks   & Extra-Search') then
    reaper.ShowMessageBox("A simple yet advanced tool for searching Items, Tracks, Take Markers, etc.,"..
    " in your session to quickly locate and navigate to them", "Searchs - Info", 0)
    end
...
reaper.ImGui_Text(ctx, "🔍 Search Tracks")
reaper.ImGui_PopStyleColor(ctx)

reaper.ImGui_SameLine(ctx, nil, 5)

-- Largeur fixe du champ de recherche
reaper.ImGui_SetNextItemWidth(ctx, 100)
local enter_pressed
enter_pressed, search_text = reaper.ImGui_InputText(ctx, "##search", search_text or "", reaper.ImGui_InputTextFlags_EnterReturnsTrue())

-- Position du champ (pour ancrer le popup)
local popup_x, popup_y = reaper.ImGui_GetItemRectMin(ctx)
local _, popup_bottom = reaper.ImGui_GetItemRectMax(ctx)

-- Si Entrée → lancer la recherche
if enter_pressed and search_text ~= "" then
  SearchItemsInTracks(search_text)
  if #search_results > 0 then
    reaper.ImGui_OpenPopup(ctx, "RésultatsRecherche")
  end
end

-- === Popup résultats ancré sous le champ ===
reaper.ImGui_SetNextWindowPos(ctx, popup_x, popup_bottom)
if reaper.ImGui_BeginPopup(ctx, "RésultatsRecherche") then
  reaper.ImGui_SetNextWindowSize(ctx, 250, 120, reaper.ImGui_Cond_Always())

  if #search_results == 0 then
    reaper.ImGui_Text(ctx, "Aucun résultat.")
  else
    for i, result in ipairs(search_results) do
      if reaper.ImGui_Selectable(ctx, result.name) then
        -- Sélection + zoom sur item
        reaper.Main_OnCommand(40289, 0)
        reaper.SetMediaItemSelected(result.item, true)
        reaper.Main_OnCommand(40913, 0)
        reaper.Main_OnCommand(reaper.NamedCommandLookup("_SWS_HZOOMITEMS"), 0)
...
if image then
      ...
      if reaper.ImGui_ImageButton(ctx, 'vignette_btn', image, 20, 20) then
        ouvrir_lien()
      end
      --reaper.ImGui_Text(ctx, "Clique sur l’image pour ouvrir le lien")

--[[
@description Move LEFT Edge of Razor Areas Backward by Nudge Value (TimecodeUI)
@version 1.0
@author Mariow
@changelog
    v1.0 (2025-12-09)
    - Initial release (moves left edge of Razor Areas backward using TimecodeUI nudge value)
@provides
    [main] ProTools_Essentials/Geeksound_MoveRazorLeftBackward-TCui.lua
@link https://github.com/Geeksound/Reaper_Scripts-Mariow
@repository https://github.com/Geeksound/Reaper_Scripts-Mariow
@tags editing, razor, nudge, items, transport, (protools-like)
@about
    # Move LEFT Edge of Razor Areas Backward by Nudge Value (ProTools-like)
    
    This script moves the **left edge** of all Razor Areas backward according to the
    ...
    ## 🟩 Usage
    - Moves the left edge of each Razor Area backward by the nudge value.  
...
local function create_razor_from_time_selection()
    ...
    -- On met le Razor Area sur TOUTES les pistes contenant la TS
    for i = 0, track_count - 1 do
        local tr = reaper.GetTrack(0, i)
...
-- Déplacer le bord gauche
local function move_left(direction)
    local delta = read_tc()
    if delta == 0 then return end

    -- Lire les Razor Areas existants
    ...
    if #areas == 0 then
        ...
        areas = get_razor_areas()       -- recharger les zones
    end
    ...
    reaper.Undo_EndBlock("Move Razor Left Backward", -1)
end
